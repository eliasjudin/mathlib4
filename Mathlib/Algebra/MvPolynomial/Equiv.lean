...
/-- Deprecated compatibility abbreviation for `uniqueAlgEquiv` specialized to `PUnit`.
...
@[deprecated "use `MvPolynomial.uniqueAlgEquiv (R := R) PUnit`" (since := "2026-04-07")]
...
theorem eval₂_uniqueAlgEquiv {σ : Type*} [Unique σ] {f : MvPolynomial σ R} {φ : R →+* S}
  ...
  | monomial d r =>
    -- Rewrite through `uniqueAlgEquiv` so that `uniqueAlgEquiv_monomial` applies.
...
/-- Simp-normal LHS for `C` through `finOneAlgEquiv` (via `finOneAlgEquiv_apply`). -/
@[simp]
lemma finOneAlgEquiv_map_C (r : R) :
    Polynomial.map ((↑(MvPolynomial.isEmptyAlgEquiv R (Fin 0)) :
        MvPolynomial (Fin 0) R →+* R))
        ((MvPolynomial.finSuccEquiv R 0) (MvPolynomial.C r)) = Polynomial.C r := by
  simp [finSuccEquiv_apply, eval₂Hom_C, Polynomial.map_C, isEmptyAlgEquiv_apply]
...
/-- Simp-normal LHS for `X 0` through `finOneAlgEquiv`. -/
@[simp]
lemma finOneAlgEquiv_map_X_zero :
    Polynomial.map ((↑(MvPolynomial.isEmptyAlgEquiv R (Fin 0)) :
        MvPolynomial (Fin 0) R →+* R))
        ((MvPolynomial.finSuccEquiv R 0) (MvPolynomial.X (0 : Fin 1))) = Polynomial.X := by
  simp [finSuccEquiv_apply, eval₂Hom_X', Polynomial.map_X]
...
/-- Coefficient of `X^n` in `finOneAlgEquiv R P` is the coefficient on `Finsupp.single 0 n`. -/
...
/-- Simp-normal LHS for `coeff_finOneAlgEquiv`; see also `finOneAlgEquiv_apply`. -/
@[simp]
lemma coeff_finOneAlgEquiv_apply (P : MvPolynomial (Fin 1) R) (n : ℕ) :
    (MvPolynomial.eval fun a => isEmptyElim a) (((MvPolynomial.finSuccEquiv R 0) P).coeff n) =
      coeff (Finsupp.single 0 n) P := by
  rw [← coeff_finOneAlgEquiv R P n]
  symm
  simp [finOneAlgEquiv, Polynomial.coeff_map, Polynomial.coe_mapAlgEquiv, isEmptyAlgEquiv_apply]
...
lemma Polynomial.toMvPolynomial_eq_rename_comp (i : σ) :
  ...
  simp [toMvPolynomial, AlgHom.comp_apply, MvPolynomial.rename_X,
    MvPolynomial.uniqueAlgEquiv_symm_apply, Polynomial.eval₂_X]
